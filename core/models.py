...
class Property(models.Model):
  ...
  """
  property_created = models.DateTimeField(
      auto_now_add=True)  # will not display
...
class ExchangeRate(models.Model):
  ...
  today = models.DateField(
      "Today's Date", auto_now_add=True)  # will not display
  OneEurCzk = models.DecimalField(
      "1 EUR - CZK", max_digits=7, decimal_places=3)
  OneEurUsd = models.DecimalField(
      "1 EUR - USD", max_digits=7, decimal_places=3)
  OneUsdCzk = models.DecimalField(
      "1 USD - CZK", max_digits=7, decimal_places=3)
...
class MyUserManager(UserManager):
  ...
  def fetch_owners_properties_count(self, user_id):
    ...
    property_count = Property.objects.filter(
        property_offered_by=property_owner).count()
