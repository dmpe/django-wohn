...
class UserProfileIndex(LoginRequiredMixin, View):
	...
	def post(self, request):
		...
		if form.is_valid():
			...
			prepare_visitor_mssg_email(request, userPresent_username = request.user.get_username(), 
				userPresent_email = request.user.email, subject="User's Feedback: ", 
				text_msg = feedback)
...
class UserProfileAdministration(LoginRequiredMixin, View):
	...
	def get(self, request):
		...
		print(gravatar_url)
		number_of_user_properties = myUser.objects.fetch_number_of_properties_per_author(user_id=request.user.id)
